...
public class DefaultTestFailure extends TestFailure {
    ...
    public static TestFailure fromTestAssertionFailure(Throwable failure) {
        DefaultTestFailureDetails details = new DefaultTestFailureDetails(failure.getMessage(), failure.getClass().getName(), stacktraceOf(failure), true);
        return new DefaultTestFailure(failure, details);
    }

    public static TestFailure fromTestFrameworkFailure(Throwable failure) {
        DefaultTestFailureDetails details = new DefaultTestFailureDetails(messageOf(failure), failure.getClass().getName(), stacktraceOf(failure), false);
        return new DefaultTestFailure(failure, details);
    }
