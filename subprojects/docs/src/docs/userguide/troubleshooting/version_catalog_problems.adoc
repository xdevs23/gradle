...
A version catalog can only import from a single source, and to prevent the import of multiple files, some methods will throw an exception when called more than once.
